...
abstract class GmthApiDistantEndpoint
{
	 ...
	 */
	protected function getJson($url, array $payload = array(), $payloadAsJson = false)
	{
		$baseRetry = 4;
		$retry = $baseRetry;
		$messageList = array();
		...
		while($retry-- > 0)
		{
			try
			{
				$contents = $this->download($url, $payload, $payloadAsJson);
		...
		throw new GmthApiException(strtr('Failed to get json data to {url} : '."\n".'{messages}', 
			array('{url}' => $url, '{messages}' => implode("\n", $messageList))),
			$lastCode);
	}
	 ...
	 */
	protected function download($url, array $payload = array(), $payloadAsJson = false)
	{
		$filteredPayload = array();
		foreach($payload as $key => $value)
		{
			if($key === '_password')
				$filteredPayload[$key] = '(Actual value removed)';
			else
		...
		while($retry-- > 0)
		{
			try
			{
				...
				$this->_logger->debug(strtr('Getting url {url} with payload {payload}.',
					array('{url}' => $url, '{payload}' => json_encode($filteredPayload))));
		...
		throw new GmthApiException('Failed to get {url} with {r} retries : '."\n".'{messages}',
			array('{url}' => $url, '{r}' => $baseRetry, '{messages}' => implode("\n", $messageList)),
			$lastCode);
	}
